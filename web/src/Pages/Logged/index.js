	...
	useEffect(
		() => {
			...
			socket.on('filter', (data) => {
				if (match.params.id !== data.hospital_id && solicitationUser !== data.user_accept) {
			...
			<div className="flex">
				...
				<span className="config">
					<div ref={changeColor}>
						...
						<label htmlFor="toggle" className="click" onClick={handleClick} ref={click}>
							Configurações
						</label>
						<ul className="info-config">
							<Link to="/update">Atualizar</Link>
							<Link to={`/changepassword/${match.params.id}`}>Trocar senha</Link>
			...
			<div className="treatments">
					...
					{ok &&
						user.map((users, i) => (
							<div className="user-help" key={i}>
								<span>
									<span style={{ fontWeight: 'bold' }}>NOME:</span> {users.user.name}
								</span>
                <span>
									<span style={{ fontWeight: 'bold' }}>CPF:</span> {users.user.cpf}
								</span>
								<span>
									<span style={{ fontWeight: 'bold' }}>DESCRIÇÃO:</span> {users.description}
								</span>
								<span>
									<span style={{ fontWeight: 'bold' }}>BIO:</span> {users.user.bio}
								</span>
								<span>
									<span style={{ fontWeight: 'bold' }}>LOCALIZAÇÃO:</span> {users.currentLocation}
								</span>
								<div>
									<button className="buttonAceitar" onClick={() => accept(users.user._id, users.description)}>Aceitar</button>
									<button className="buttonRecusar" onClick={() => reject(users.user._id)}>Recusar</button>
								</div>
							</div>
						))}
				</div>
                ...
                <span>
								...
								<div>
									<button
										onClick={() => {
											setTest(testUser.filter((usr) => usr.user._id !== users.user._id));
											setArrived(true);
                ...
                <span>
								...
								<div>
									<button onClick={() => hasAttended(user._id)}>Atendido</button>
								</div>
							</div>
						))}
				</div>
